...
# Usage: Scripts/attach-release-asset.sh v0.1.2 ./path/to/Lumiere-0.1.2.dmg [./path/to/Lumiere-0.1.2.zip ...]
set -euo pipefail

TAG="${1:-}"
shift || true
REPO="${REPO:-nodaysidle/nodaysidle-lumiere}"
...
if ! gh release view "$TAG" --repo "$REPO" >/dev/null 2>&1; then
  ...
  echo "  gh release create \"$TAG\" --repo \"$REPO\" --generate-notes --title \"Lumiere $TAG\"" >&2
  exit 1
fi
